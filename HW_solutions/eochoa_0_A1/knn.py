...
def compute_distances_two_loops(x_train: torch.Tensor, x_test: torch.Tensor):
    ...
    for i in range(num_train):
      for j in range(num_test):
        dists[i,j] = ((x_train_f[i] - x_test_f[j]) ** 2).sum().item() ** 0.5
    ##########################################################################
    #                           END OF YOUR CODE                             #
    ##########################################################################
    return dists
...
def compute_distances_one_loop(x_train: torch.Tensor, x_test: torch.Tensor):
    ...
    x_test_f = x_test.view(num_test, -1)
    for i in range(num_train):
      dists[i, :] = ((x_train_f[i] - x_test_f) ** 2).sum(1).view(1, num_test) ** 0.5
    ##########################################################################
    #                           END OF YOUR CODE                             #
    ##########################################################################
    return dists
...
def compute_distances_no_loops(x_train: torch.Tensor, x_test: torch.Tensor):
    ...
    x_train_f = x_train.view(num_train, -1)
    x_test_f = x_test.view(num_test, -1)
    dists = ((x_train_f ** 2).sum(1).view(num_train, 1) + 
    (x_test_f ** 2).sum(1).view(1, num_test) - 
    2 * torch.matmul(x_train_f, x_test_f.t())) ** 0.5
    ##########################################################################
    #                           END OF YOUR CODE                             #
    ##########################################################################
    return dists
...
def predict_labels(dists: torch.Tensor, y_train: torch.Tensor, k: int = 1):
    ...
    ##########################################################################
    # Replace "pass" statement with your code
    idxs = dists.topk(k=k, dim=0, largest=False)[1]
    y_pred = torch.mode(y_train[idxs],0)[0]
    ##########################################################################
    #                           END OF YOUR CODE                             #
    ##########################################################################
    return y_pred
...
def knn_cross_validate(
    ...
    # Replace "pass" statement with your code
    #torch.manual_seed(0)
    x_train_folds = list(torch.chunk(x_train, chunks=num_folds))
    y_train_folds = list(torch.chunk(y_train, chunks=num_folds))
    ##########################################################################
    #                           END OF YOUR CODE                             #
    ##########################################################################
    ...
    for k in k_choices:
      ...
      for i in range(num_folds):
        num_folds_list = list(range(num_folds))
        num_folds_list.pop(i)
        x_train_fold = [x_train_folds[j] for j in num_folds_list]
        y_train_fold = [y_train_folds[j] for j in num_folds_list]
        x = torch.cat(x_train_fold)
        y = torch.cat(y_train_fold)
...
def knn_get_best_k(k_to_accuracies: Dict[int, List]):
    ...
    ##########################################################################
    # Replace "pass" statement with your code
    keys = list(k_to_accuracies.keys())
    k = keys[0]
    best_k = k
    best_a = sum(k_to_accuracies[k]) / len(k_to_accuracies[k])
    for k in keys[1:]:
      if best_a < sum(k_to_accuracies[k]) / len(k_to_accuracies[k]):
        best_a = sum(k_to_accuracies[k]) / len(k_to_accuracies[k])
        best_k = k
