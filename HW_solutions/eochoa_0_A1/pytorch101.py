...
def mutate_tensor(
    ...
    ##########################################################################
    # Replace "pass" statement with your code
    i_1 = [i[0] for i in indices]
    i_2 = [i[1] for i in indices]
    x[i_1, i_2] = torch.tensor(values)
    ##########################################################################
    #                            END OF YOUR CODE                            #
    ##########################################################################
    return x
...
def count_tensor_elements(x: Tensor) -> int:
    ...
    ##########################################################################
    # Replace "pass" statement with your code
    num_elements = torch.prod(torch.tensor(x.shape), 0).item()
    ##########################################################################
    #                            END OF YOUR CODE                            #
    ##########################################################################
    return num_elements
...
def multiples_of_ten(start: int, stop: int) -> Tensor:
    ...
    ##########################################################################
    # Replace "pass" statement with your code
    start_m = (int(start / 10) + 1) * 10
    #stop_m = (int(stop / 10)) * 10
    stop_m = (int(stop / 10) + 1) * 10

    if start_m >= stop_m:
      return torch.tensor([], dtype=torch.float64)

    x = torch.arange(start=start_m, end=stop_m, step=10, dtype=torch.float64)
    ##########################################################################
    #                            END OF YOUR CODE                            #
    ##########################################################################
    return x
...
def take_one_elem_per_col(x: Tensor) -> Tensor:
    ...
    ##########################################################################
    # Replace "pass" statement with your code
    idx0 = torch.tensor([1, 0, 3]) #[1, 0, 3]
    idx1 = torch.tensor([0, 1, 2]) #[0, 1, 2]
    y = x[idx0, idx1]
    ##########################################################################
    #                            END OF YOUR CODE                            #
    ##########################################################################
...
def make_one_hot(x: List[int]) -> Tensor:
    ...
    n_max = max(x)
    n_len = len(x)
    y = torch.zeros((n_len, n_max + 1))
    idx0 = torch.arange(len(x))
    idx1 = torch.tensor(x)# - 1
    y[idx0, idx1] = 1
    ##########################################################################
    #                            END OF YOUR CODE                            #
    ##########################################################################
...
def zero_row_min(x: Tensor) -> Tensor:
    ...
    # Replace "pass" statement with your code
    y = x.clone()
    y[torch.arange(x.shape[0]), x.argmin(1)] = 0
    ##########################################################################
    #                            END OF YOUR CODE                            #
    ##########################################################################
    return y
...
def normalize_columns(x: Tensor) -> Tensor:
    ...
    # Replace "pass" statement with your code
    dim_x = x.shape[0]
    mu = x.to(torch.float64).sum(dim=0) / dim_x
    sigma = (((x - mu) ** 2).sum(dim=0) / (dim_x - 1)) ** 0.5
    y = (x - mu) / sigma
...
def challenge_mean_tensors(xs: List[Tensor], ls: Tensor) -> Tensor:
    ...
    ##########################################################################
    # Replace "pass" statement with your code
    ls_c = torch.tensor(ls).cumsum(0)
    xs_c = torch.cat(xs, dim=0).cumsum(0)
    y = torch.cat([xs_c[ls_c - 1][0:1], xs_c[ls_c - 1][1:] - xs_c[ls_c - 1][:-1]]) / ls
    ##########################################################################
    #                            END OF YOUR CODE                            #
    ##########################################################################
    return y
