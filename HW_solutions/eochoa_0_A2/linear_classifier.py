...
def svm_loss_vectorized(
    ...
    num_train = X.shape[0]
    scores = W.t().mm(X.t())
    correct_class_score = scores[y, list(range(y.shape[0]))]
    loss_mat = scores - correct_class_score + 1
    pos_scores = loss_mat > 0
    loss_mat = loss_mat * pos_scores
    loss = (loss_mat.sum() - num_train) / num_train
    #############################################################################
    #                             END OF YOUR CODE                              #
    #############################################################################
    ...
    pos_scores = (pos_scores * 1.0).to(torch.float64)
    pos_scores[y, list(range(y.shape[0]))] = - (num_classes - 1) * pos_scores[y, list(range(y.shape[0]))]
    pos_scores = pos_scores.t()
    dW = X.t().mm(pos_scores) / num_train
    dW += reg * 2 * W
    #############################################################################
    #                             END OF YOUR CODE                              #
    #############################################################################
...
def svm_get_search_params():
    ...
    ###########################################################################
    # Replace "pass" statement with your code
    learning_rates = torch.linspace(start=5.050488e-04, end=5.115510e-04, steps=10)
    regularization_strengths = [0]
    #learning_rates = [3e-11] * 5
    #regularization_strengths = [2.5e4] * 5
    ###########################################################################
    #                           END OF YOUR CODE                              #
    ###########################################################################
...
def softmax_loss_naive(
    ...
    for i in range(num_train):
        ...
        correct_class_score = scores_norm[y[i]]
        L = -torch.log(torch.exp(correct_class_score) / torch.exp(scores_norm).sum())
        loss += L
        #one_aux_1 = torch.zeros_like(X[:,0:1])
        #one_aux_1[0, y[i]] = 1
        one_aux = torch.zeros_like(W[0:1,:])
        one_aux[0, y[i]] = 1
        dW += -X.t()[:,i:(i+1)].sum(1, keepdim=True).mm(one_aux)
        for j in range(num_classes):
          one_aux = torch.zeros_like(W[0:1,:])
          one_aux[0, j] = 1
          dW = dW + torch.exp(scores[j]) * X.t()[:,i:(i+1)].sum(1, keepdim=True).mm(one_aux) / torch.exp(scores).sum()
    # Right now the loss is a sum over all training examples, but we want it
    # to be an average instead so we divide by num_train.
    loss /= num_train
...
def softmax_loss_vectorized(
    ...
    aux_zero_mat = W.new_zeros(num_train, num_classes)
    aux_zero_mat[list(range(num_train)), y] = 1
    
    scores = W.t().mm(X.t())
    scores_max = scores.max(0).values
    exp_scores = torch.exp(scores)

    scores_norm = scores - scores_max
    correct_class_score = scores_norm[y, list(range(num_train))]
    loss = -torch.log(torch.exp(correct_class_score) / torch.exp(scores_norm).sum(0)).sum()
    #loss = (-correct_class_score + torch.log(1e-10 + exp_scores.sum(0))).sum()
    loss /= num_train

    dW += X.t().mm((exp_scores / exp_scores.sum(0)).t() - aux_zero_mat)
    dW = dW / num_train
    dW += reg * 2 * W
