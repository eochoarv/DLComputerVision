...
def nn_forward_pass(params: Dict[str, torch.Tensor], X: torch.Tensor):
    ...
    hidden = X.mm(W1) + b1
    hidden_pos = X.mm(W1) + b1 > 0
    hidden = hidden * hidden_pos
...
def nn_forward_backward(
    ...
    ############################################################################
    # Replace "pass" statement with your code
    scores_correct = scores[list(range(N)), y]
    exp_scores = torch.exp(scores)

    scores_max = scores.max(1, keepdim=True).values
    scores_norm = (scores - scores_max)
    exp_scores_norm = torch.exp(scores_norm)
    correct_class_score_norm = scores_norm[list(range(N)), y]
    exp_correct_class_score_norm = exp_scores_norm[list(range(N)), y]
    loss = (-correct_class_score_norm + torch.log(exp_scores_norm.sum(1))).sum()

    #loss = -torch.log(torch.exp(scores_correct) / exp_scores.sum(1)).sum()
    loss = loss / N
    ...
    if y == None:
      ...
      H, C = W2.shape
      grad_L = 1
      grad_neg_log = grad_L * torch.ones_like(X[:,0]) / N # N x 1
      grad_sum_exp_scores = (torch.diag(torch.exp(0 - scores_max.reshape((N,))) / torch.exp(scores - scores_max).sum(1))).mv(grad_neg_log)  
      #grad_sum_exp_scores = (torch.diag(1 / exp_scores.sum(1))).mv(grad_neg_log)
      #print(grad_sum_exp_scores)
      grad_exp_scores = torch.outer(grad_sum_exp_scores, torch.ones(C, device=X.device))
      grad_neg_socores_correct = grad_neg_log
      _aux = torch.zeros_like(scores)
      _aux[list(range(N)), y] = 1
      grad_scores = (grad_exp_scores) - (_aux * grad_neg_socores_correct.reshape((N,1)))
      grad_b2 = grad_scores.sum(0)
      grad_W2 = h1.t().mm(grad_scores) + reg * 2 * W2
      grad_H = grad_scores.mm(W2.t())
      grad_H_scores = grad_H * (X.mm(W1) + b1 > 0)
      grad_b1 = grad_H_scores.sum(0)
      grad_XW1 = grad_H_scores
      grad_W1 = X.t().mm(grad_XW1) + reg * 2 * W1
...
def nn_get_search_params():
    ...
    ###########################################################################
    # Replace "pass" statement with your code
    learning_rates = [0.005, 0.01, 0.015, 0.02]#torch.linspace(start=0.005, end=0.02, steps=10)
    hidden_sizes = [512, 768, 1024]
    regularization_strengths = [0.000005, 0.005, 0.05]
    learning_rate_decays = [0.93, 0.94, 0.95]
    ###########################################################################
    #                           END OF YOUR CODE                              #
    ###########################################################################
...
def find_best_net(
    ...
    #############################################################################
    # Replace "pass" statement with your code
    lrs, hidden_sizes, regs, learning_rate_decay = get_param_set_fn()
    #param_comb = 0
    for lrd in learning_rate_decay:
      for hs in hidden_sizes: 
        for reg in regs:
          for lr in lrs:
            #print('train with param:')
            print((lrd, hs, reg, lr))
            ...
            max_acc = torch.tensor(stats['val_acc_history']).max()
            print('val acc:{}'.format(max_acc))
            if best_val_acc < max_acc:
              best_val_acc = max_acc
              best_epoch = torch.tensor(stats['val_acc_history']).argmax()
              best_net = net
              best_stat = stats
            
            #stat_dict[param_comb] = stats
            #param_comb += 1
