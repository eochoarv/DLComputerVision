...
class Linear(object):
    ...
    def backward(dout, cache):
        ...
        dw = x.reshape((N, -1)).t().mm(dout) #+ reg * 2 * W2
        db = dout.sum(0)
...
class TwoLayerNet(object):
    ...
    def loss(self, X, y=None):
        ...
        N, D = X.shape
        H, cache_H = Linear_ReLU.forward(X, W1, b1)
        #cache_H0, cache_H1 = cache_H
        scores, cache_scores = Linear.forward(H, W2, b2)
        if y == None:
          return scores
        ##############################################################
        #                     END OF YOUR CODE                       #
        ...
        dH, dW2, db2 = Linear.backward(dL, cache_scores)
        dX, dW1, db1 = Linear_ReLU.backward(dH, cache_H)
...
class FullyConnectedNet(object):
    ...
    def __init__(self, hidden_dims, input_dim=3*32*32, num_classes=10,
        ...
        for i in range(L + 1):
          ...
          self.params["W"+str(i+1)] = weight_scale * torch.randn(
            prev_dim, hidden_dim, dtype=dtype, device=device)

          self.params["b"+str(i+1)] = torch.zeros(hidden_dim, 
    ...
    def loss(self, X, y=None):
        ...
        for i in range(L - 1):
          ...
          if i == 0:
            H, cache_h = Linear_ReLU.forward(X, Wi, bi)
          else:
            H, cache_h = Linear_ReLU.forward(layers[i-1], Wi, bi)
        ...
        scores, cache_scores = Linear.forward(layers[-1], Wi, bi)
        if self.use_dropout:
        ...
        for i in range(L):
          ...
          if i == 0:
            dH, dWi, dbi = Linear.backward(dout, cache_out)
          else:
            dH, dWi, dbi = Linear_ReLU.backward(dout, cache_out)
          dout = dH
...
def create_solver_instance(data_dict, dtype, device):
    ...
    solver = Solver(model, 
    data_dict, 
    optim_config={'learning_rate': 5e-1},
    print_every=100,
    num_epochs=15, 
    batch_size=500,
    lr_decay=0.99,
...
def rmsprop(w, dw, config=None):
    ...
    w += - config['learning_rate'] * dw / (torch.sqrt(cache) + config['epsilon'])
    next_w = w
    config['cache'] = cache
    ###########################################################################
    #                             END OF YOUR CODE                            #
...
def adam(w, dw, config=None):
    ...
    #print(vt.shape)
    #print(w.shape)
    w += - learning_rate * mt / (torch.sqrt(vt) + epsilon)
...
class Dropout(object):
    ...
    def forward(x, dropout_param):
        ...
        if mode == 'train':
            ...
            ##############################################################
            # Replace "pass" statement with your code
            mask = (torch.rand(*x.shape, device=x.device) > p) / (1-p) # first dropout mask. Notice /p!
            out = x * mask # drop!
            ##############################################################
            #                   END OF YOUR CODE                         #
            ##############################################################
